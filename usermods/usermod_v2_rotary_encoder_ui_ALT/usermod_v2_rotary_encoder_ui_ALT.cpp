...
class RotaryEncoderUIUsermod : public Usermod {
  ...
  private:
    ...
    static const char _stateDefault[];
  ...
  public:
    ...
    RotaryEncoderUIUsermod()
      ...
      , stateDefault(SELECT_STATE_DEFAULT)
    {}
    ...
    bool changeState(const char *stateName, byte markedLine, byte markedCol, byte glyph, bool overlay);
    bool setNewState(unsigned char &newState, bool overlay = true);
    void lampUdated();
...
bool RotaryEncoderUIUsermod::setNewState(unsigned char &newState, bool overlay) {
  ...
  if (display != nullptr) {
    switch (newState) {
      case  0: changedState = changeState(lineBuffer,   1,   0,  1, overlay); break; //1  = sun
      case  1: changedState = changeState(lineBuffer,   1,   4,  2, overlay); break; //2  = skip forward
      case  2: changedState = changeState(lineBuffer,   1,   8,  3, overlay); break; //3  = fire
      case  3: changedState = changeState(lineBuffer,   2,   0,  4, overlay); break; //4  = custom palette
      case  4: changedState = changeState(lineBuffer,   3,   0,  5, overlay); break; //5  = puzzle piece
      case  5: changedState = changeState(lineBuffer, 255, 255,  7, overlay); break; //7  = brush
      case  6: changedState = changeState(lineBuffer, 255, 255,  8, overlay); break; //8  = contrast
      case  7: changedState = changeState(lineBuffer, 255, 255, 10, overlay); break; //10 = star
      case  8: changedState = changeState(lineBuffer, 255, 255, 11, overlay); break; //11 = heart
      case  9: changedState = changeState(lineBuffer, 255, 255, 10, overlay); break; //10 = star
      case 10: changedState = changeState(lineBuffer, 255, 255, 10, overlay); break; //10 = star
      case 11: changedState = changeState(lineBuffer, 255, 255, 10, overlay); break; //10 = star
    }
  }
  return changedState;
}
...
void RotaryEncoderUIUsermod::loop()
{
  ...
  if (currentTime >= (loopTime + 2)) // 2ms since last check of encoder = 500Hz
  {
    ...
    if (buttonWaitTime && currentTime-buttonWaitTime>350 && !buttonPressedBefore) { //same speed as in button.cpp
      buttonWaitTime = 0;
      unsigned char newState = select_state + 1;
      if (setNewState(newState)) select_state = newState;
    }
...
bool RotaryEncoderUIUsermod::changeState(const char *stateName, byte markedLine, byte markedCol, byte glyph, bool overlay) {
#ifdef USERMOD_FOUR_LINE_DISPLAY
  if (display != nullptr) {
    ...
    if (overlay) display->overlay(stateName, 750, glyph);
...
void RotaryEncoderUIUsermod::addToConfig(JsonObject &root) {
  ...
  top[FPSTR(_stateDefault)] = stateDefault;
...
bool RotaryEncoderUIUsermod::readFromConfig(JsonObject &root) {
  ...
  unsigned char newSelectState = top[FPSTR(_stateDefault)] | stateDefault;
...
const char RotaryEncoderUIUsermod::_stateDefault[] PROGMEM = "state-default";
